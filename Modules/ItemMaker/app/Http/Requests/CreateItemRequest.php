...
class CreateItemRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        $this->merge([
            ...
            'requires_preparation' => $this->input('requires_preparation', true),
            'display_order' => $this->input('display_order', 0),
        ]);

        if ($this->filled('ingredients')) {
        ...
        if ($this->filled('boxes')) {
            $boxes = $this->input('boxes', []);
            foreach ($boxes as $key => $box) {
                if (!isset($box['is_default_packaging'])) {
                    $boxes[$key]['is_default_packaging'] = $key === 0;
                }
